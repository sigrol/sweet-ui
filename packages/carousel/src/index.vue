<template>
  ...
  >
    <div class="s-carousel__container" :style="{ height: height }">
      <transition v-if="arrowDisplay" name="carousel-arrow-left">
        <button
          ...
          "
            type="button"
            class="s-carousel__arrow s-carousel__arrow--left"
            @mouseenter="handleButtonEnter('left')"
            @mouseleave="handleButtonLeave"
            @click.stop="throttledArrowClick(data.activeIndex - 1)"
        >
          <s-icon name="sweet-icon-arrow-left"></s-icon>
        </button>
      </transition>
      <transition v-if="arrowDisplay" name="carousel-arrow-right">
        <button
          ...
          "
            type="button"
            class="s-carousel__arrow s-carousel__arrow--right"
            @mouseenter="handleButtonEnter('right')"
            @mouseleave="handleButtonLeave"
            @click.stop="throttledArrowClick(data.activeIndex + 1)"
        >
          <s-icon name="sweet-icon-arrow-right"></s-icon>
        </button>
      </transition>
      <slot></slot>
    </div>
    ...
    <ul v-if="indicatorPosition !== 'none'" :class="indicatorsClasses">
      <li
          ...
          :key="index"
          :class="[
          's-carousel__indicator',
          's-carousel__indicator--' + direction,
          { 'is-active': index === data.activeIndex },
        ]"
          @mouseenter="throttledIndicatorHover(index)"
          @click.stop="handleIndicatorClick(index)"
      >
        <button class="s-carousel__button">
...
export default defineComponent({
  ...
  setup(props:ICarouselProps, {emit}) {
    ...
    const carouselClasses = computed(() => {
      const classes = ['s-carousel', 's-carousel--' + props.direction]
      if (props.type === 'card') {
        classes.push('s-carousel--card')
      }
      return classes
    })
    ...
    const indicatorsClasses = computed(() => {
      const classes = [
        ...
        's-carousel__indicators--' + props.direction,
      ]
      if (hasLabel.value) {
        classes.push('s-carousel__indicators--labels')
      }
      if (props.indicatorPosition === 'outside' || props.type === 'card') {
        classes.push('s-carousel__indicators--outside')
      }
      return classes
    })
    ...
    function setActiveItem(index) {
      ...
      if (isNaN(index) || index !== Math.floor(index)) {
        console.warn('[SWEET Warn][Carousel]index must be an integer.')
        return
      }
